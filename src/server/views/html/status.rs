...
fn dependency_table(title: &str, deps: &IndexMap<CrateName, AnalyzedDependency>) -> Markup {
    ...
    html! {
        ...
        p class="subtitle is-5" {
            (match count_outdated {
                0 => format!("({count_total} total, all up-to-date)"),
                _ => format!("({count_total} total, {count_outdated} outdated)"),
            })
        }
...
/// Renders a warning with the numbers of outdated main and development dependencies.
/// Renders nothing if both counts are zero.
